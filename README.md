...
## Арифметическое кодирование с LLM

**Арифметическое кодирование** — это метод энтропийного сжатия, который последовательно сужает интервал [0,1) в зависимости от текущего предсказанного распределения символов.
Вместо фиксированных кодовых слов мы кодируем всю последовательность токенов как единственное число — итоговый битовый префикс. Границы этого числа последовательно сужаются в зависимости от предсказанного LLM распределения.

### Алгоритм

1. **Инициализация**
   Устанавливаем два конца интервала:
...
2. **Основной цикл (для каждого токена tᵢ)**

   * **Предсказание.** Передаём LLM уже закодированные токены и получаем вектор вероятностей
     `probs = [p₀, p₁, …, p_{V−1}]`, где ∑ p = 1.
   * **Кумулята.** Строим кумулятивную функцию
     `CDF[k] = ∑_{j<k} pⱼ`.
   * **Сужение интервала.** Обозначим `R = high − low`. Тогда

     ```
     low  = low  + ⌊R * CDF[tᵢ]    ⌋
     high = low  + ⌊R * CDF[tᵢ + 1]⌋
     ```
   * **Нормализация.** Если `low` и `high` попадают в «крайние» полубайты, сдвигаем их биты, сохраняя точность (E1/E2/E3).

3. **Завершение**
   Выбираем любое число внутри финального `[low, high)` и выводим его битовый префикс длины `precision`.

4. **Декодирование**
   Декодер читает первые `precision` бит в число `value`, затем для каждого токена повторяет:

   * находит `tᵢ` по условию

     ```
     CDF[tᵢ] ≤ (value−low)/(high−low) < CDF[tᵢ+1]
     ```
   * сужает интервал тем же способом, что и энкодер, и нормализует.
...
Эмпирический prior использует больше метаданных (8 320 бит) и даёт наибольший общий объём данных (804 596 бит) с наибольшей MSE (4.49 × 10⁻³), то является неэффективным.
...
Получившийся steering vector в целях дальнейшей оптимизации можно сам сжать, использую подход на основе hyper prior. Для лучшего коэффициента сжатия steering vector при малом объеме данных лучше использовать гауссов гипер-приор, а для лучшей точности восстановления steering vector - Lloyd–Max гипер‑приор.
