...
Добавление фиксированной текстовой строки или статического soft‑prompt чуть ухудшает коэффициент по сравнению с базовым (0.1658), тогда как оптимизация soft‑prompt в ходе обучения (100 и особенно 500 эпох) заметно улучшает сжатие, снижая коэффициент до 0.1356.
...
* **Гауссов** приор даёт наименьшую энтропию (6.725 бит/коорд) и требует наименьшее число бит (6.741 бит/коорд), но MSE у него выше, чем у Lloyd–Max. При этом коэффициент сжатия steering‑вектора самый низкий (0.2106), и итоговое сжатие текста тоже близко к лучшему (0.1355).

* **Эмпирический** приор оказывается самым «разболтанным»: наибольшее число бит на координату (7.7645 бит), высокая энтропия и крупная MSE. Соответственно, коэффициент компрессии steering‑вектора худший (0.2426), а размер текста после сжатия чуть хуже остальных (0.1367).

* **Lloyd–Max** сочетает почти оптимальную энтропию (7.703 бит/коорд) с крайне низкой MSE (4·10⁻⁵), немного уступая по общему числу бит эмпирическому, но превосходя его по качеству восстановления. Его коэффициент сжатия steering‑вектора (0.2407) приближается к эмпирическому, а итоговый коэффициент сжатия текста — лучший (0.1354).
