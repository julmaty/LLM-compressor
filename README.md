...
## Арифметическое кодирование с использованием LLM

**Арифметическое кодирование** — это метод энтропийного сжатия, который последовательно сужает интервал $[0,1)$ в зависимости от текущего предсказанного распределения символов. Вместо жёсткого присвоения фиксированных кодовых слов каждому токену, мы кодируем поток токенов как единственное вещественное число в интервале, постепенно «приценивая» вероятности.

С LLM (большими языковыми моделями) схема выглядит так:

1. **Инициализация**

   * Заводим `low = 0`, `high = 2^{precision}−1`.

2. **Итеративная сужающаяся кодировка**
   Для каждого следующего токена $t_i$ по порядку:

   1. Передаём в LLM префикс $(t_0, t_1, …, t_{i-1})$.
   2. Получаем предсказанное распределение вероятностей

      $$
        p_0, p_1, \dots, p_{V-1}, \quad \sum_k p_k = 1
      $$

      по словарю размера $V$.
   3. Вычисляем кумулятивную функцию:
      ...
      $$
        ...
        C(V) = 1.
      $$
   4. Сдвигаем границы текущего интервала $[\,\text{low},\,\text{high}\,)$ в соответствии с выбором токена $t_i$:

      $$
        \text{new\_range} = \text{high}-\text{low},
      $$

      $$
        \text{high} = \text{low} + \text{new\_range}\times C(t_i+1),
        \quad
        \text{low}  = \text{low} + \text{new\_range}\times C(t_i).
      $$
   5. При необходимости делаем сдвиг битов, чтобы держать значения в рамках точности.

3. **Завершение**
   После кодирования последнего токена выбираем любое число внутри окончательного интервала и представляем его битовой строкой. Это и есть наш сжатый поток.

4. **Декодирование**
   Декодер, получив начальный битовый префикс, восстанавливает токены по тем же самым предсказаниям LLM:

   1. Поддерживает те же `low`, `high` и внутреннее значение `value` (считанное из бит).
   2. В каждом шаге находит $t_i$ такое, что

      $$
        C(t_i) \le \frac{\text{value}-\text{low}}{\text{high}-\text{low}} < C(t_i+1).
      $$
   3. Обновляет `low`/`high` как в энкодере и продолжает.
...
| Гипер‑приор            | Битов для данных | Бит/коорд | Метаданных (бит) | Всего бит | Энтр./коорд (бит) | Энтр. всего (бит) | MSE           | Коэф. сжатия |
| ---------------------- | ---------------: | --------: | ---------------: | --------: | ----------------: | ----------------: | ------------- | -----------: |
| Гауссов                |          690 204 |    6.7403 |              192 |   690 396 |            6.7252 |        688 655.97 | 5.107074 e-04 |       0.2107 |
| Эмпирический           |          796 276 |    7.7761 |            8 320 |   804 596 |            7.7136 |        789 871.63 | 4.488599 e-03 |       0.2455 |
| Lloyd–Max (1D k‑means) |          787 205 |    7.6875 |           16 480 |   803 685 |            7.6872 |        787 172.48 | 4.065110 e-05 |       0.2453 |

Гауссов prior требует минимум метаданных (192 бит) и даёт самый низкий общий объём данных (690 396 бит), однако имеет не самую низкую MSE (5.1 × 10⁻⁴).
Эмпирический prior использует больше метаданных (8 320 бит) и даёт наибольший общий объём данных (804 596 бит) с наибольшей MSE (4.49 × 10⁻³), то есть не является эффективным ни в каком отношении.
В Lloyd–Max prior метаданные (16 480 бит) занимают вдвое больше, чем у эмпирического приора, но итоговый объём (803 685 бит) практически равен эмпирическому, так как MSE рекордно малая (4.07 × 10⁻⁵), что обеспечивает более эффективное сжатие основного текста.

<img src="https://github.com/julmaty/LLM-compressor/blob/main/Diagramm.png"
     alt="Rate - distortion диаграмма"
     width="50%" />
